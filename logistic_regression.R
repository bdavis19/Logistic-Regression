...
# Build a model
everwk.mod <- glm(everwrk ~ age_p + r_maritl, data=NH11, family="binomial")

# Get out of log odds
coef(summary(everwk.mod))
everwk.mod.tab <- coef(summary(everwk.mod))
everwk.mod.tab[, "Estimate"] <- exp(coef(everwk.mod))
everwk.mod.tab
